...
noncomputable def alpha (k : ℝ) : ℝ :=
  if h : k > 1 then
    Classical.choose (show ∃ x > 0, x * (1 + x) ^ (k - 1) = 1 by
                        -- By the Intermediate Value Theorem, since $f(x) = x(1 + x)^{k - 1}$ is continuous on $[0, \infty)$ and $f(0) = 0$ and $f(1) = 2^{k - 1} > 1$, there exists some $c \in (0, 1)$ such that $f(c) = 1$.
                        have h_ivt : ∃ c ∈ Set.Ioo 0 1, (fun x : ℝ => x * (1 + x) ^ (k - 1)) c = 1 := by
                          apply_rules [ intermediate_value_Ioo ] <;> norm_num;
                          · exact continuousOn_of_forall_continuousAt fun x hx => ContinuousAt.mul continuousAt_id <| ContinuousAt.rpow ( continuousAt_const.add continuousAt_id ) continuousAt_const <| Or.inl <| by linarith [ hx.1 ] ;
...
lemma lemma_l2 (k : ℝ) (hk : k > 1) : alpha k > 1 / k := by
  ...
  have h_f_alpha_k : f (alpha k) = 1 := by
    convert Classical.choose_spec ( _ : ∃ x : ℝ, x > 0 ∧ x * ( 1 + x ) ^ ( k - 1 ) = 1 ) |> And.right using 1;
    rw [ show alpha k = Classical.choose ( _ : ∃ x : ℝ, x > 0 ∧ x * ( 1 + x ) ^ ( k - 1 ) = 1 ) from ?_ ];
    rw [alpha, dif_pos hk]; -- Unfold alpha and resolve the dependent if
  ...
  exact mul_le_mul ( by nlinarith [ mul_div_cancel₀ 1 ( by linarith : k ≠ 0 ) ] ) ( Real.rpow_le_rpow ( by nlinarith [ mul_div_cancel₀ 1 ( by linarith : k ≠ 0 ), show 0 ≤ alpha k from by unfold alpha; split_ifs ; first | positivity | exact Classical.choose_spec ( _ : ∃ x : ℝ, x > 0 ∧ x * ( 1 + x ) ^ ( k - 1 ) = 1 ) |>.1.le ] ) ( by nlinarith [ mul_div_cancel₀ 1 ( by linarith : k ≠ 0 ), show 0 ≤ alpha k from by unfold alpha; split_ifs ; first | positivity | exact Classical.choose_spec ( _ : ∃ x : ℝ, x > 0 ∧ x * ( 1 + x ) ^ ( k - 1 ) = 1 ) |>.1.le ] ) ( by linarith ) ) ( by exact Real.rpow_nonneg ( by nlinarith [ mul_div_cancel₀ 1 ( by linarith : k ≠ 0 ), show 0 ≤ alpha k from by unfold alpha; split_ifs ; first | positivity | exact Classical.choose_spec ( _ : ∃ x : ℝ, x > 0 ∧ x * ( 1 + x ) ^ ( k - 1 ) = 1 ) |>.1.le ] ) _ ) ( by nlinarith [ mul_div_cancel₀ 1 ( by linarith : k ≠ 0 ), show 0 ≤ alpha k from by unfold alpha; split_ifs ; first | positivity | exact Classical.choose_spec ( _ : ∃ x : ℝ, x > 0 ∧ x * ( 1 + x ) ^ ( k - 1 ) = 1 ) |>.1.le ] )
...
lemma lemma_l1_one (k : ℝ) (hk : k > 1) :
  ...
  suffices h_subst : Filter.Tendsto (fun u => ((1 - u) ^ (k - 1) * ((1 - u) * Real.log (1 - u)⁻¹ + u * Real.log u⁻¹)) / ((1 - u) ^ k * Real.log ((1 - u) ^ k)⁻¹ + (1 - (1 - u) ^ k) * Real.log (1 - (1 - u) ^ k)⁻¹)) (nhdsWithin 0 (Set.Ioi 0)) (nhds (1 / k)) by
    ...
    intro ε hε; rcases h_subst ε hε with ⟨ δ, hδ, H ⟩ ; exact ⟨ δ, hδ, fun { x } hx₁ hx₂ => by convert H ( show 0 < 1 - x from by linarith [ hx₁.out ] ) ( by simpa [ dist_comm ] using hx₂ ) using 1; ring ⟩ ;
  ...
  have h_approx : Filter.Tendsto (fun u => (1 - u) ^ (k - 1) * ((1 - u) * Real.log (1 - u)⁻¹ + u * Real.log u⁻¹) / (u * Real.log u⁻¹)) (nhdsWithin 0 (Set.Ioi 0)) (nhds 1) ∧ Filter.Tendsto (fun u => ((1 - u) ^ k * Real.log ((1 - u) ^ k)⁻¹ + (1 - (1 - u) ^ k) * Real.log (1 - (1 - u) ^ k)⁻¹) / (u * Real.log u⁻¹)) (nhdsWithin 0 (Set.Ioi 0)) (nhds k) := by
    ...
    · -- We'll use the fact that as $u \to 0$, $(1 - u)^k \approx 1 - ku$ and $\log(1 - (1 - u)^k) \approx \log(ku)$.
      ...
      convert h_approx.add ( ‹Filter.Tendsto ( fun u : ℝ => ( 1 - ( 1 - u ) ^ k ) / u ) ( nhdsWithin 0 ( Set.Ioi 0 ) ) ( nhds k ) ∧ Filter.Tendsto ( fun u : ℝ => Real.log ( 1 - ( 1 - u ) ^ k ) / Real.log u ) ( nhdsWithin 0 ( Set.Ioi 0 ) ) ( nhds 1 ) ›.1.mul ‹Filter.Tendsto ( fun u : ℝ => ( 1 - ( 1 - u ) ^ k ) / u ) ( nhdsWithin 0 ( Set.Ioi 0 ) ) ( nhds k ) ∧ Filter.Tendsto ( fun u : ℝ => Real.log ( 1 - ( 1 - u ) ^ k ) / Real.log u ) ( nhdsWithin 0 ( Set.Ioi 0 ) ) ( nhds 1 ) ›.2 ) using 2 <;> ring;
...
lemma one_div_G_eq (x : ℝ) (hx : 0 < x ∧ x < 1) :
  ...
  · rw [ Real.exp_log hx.1, Real.exp_log ( by linarith ) ] ; unfold Real.binEntropy ; ring;
...
lemma q_deriv_eq_zero_iff (k : ℝ) (hk : k > 1) (x : ℝ) (hx : 0 < x ∧ x < 1) :
  ...
  have h_eq : deriv (q k) x = 0 ↔ ((k - 1) * Real.binEntropy x + x * deriv Real.binEntropy x) * Real.binEntropy (x ^ k) = k * x ^ k * Real.binEntropy x * deriv Real.binEntropy (x ^ k) := by
    have h_eq : deriv (q k) x = ((k - 1) * x ^ (k - 2) * Real.binEntropy x + x ^ (k - 1) * deriv Real.binEntropy x) * Real.binEntropy (x ^ k) / Real.binEntropy (x ^ k) ^ 2 - x ^ (k - 1) * Real.binEntropy x * k * x ^ (k - 1) * deriv Real.binEntropy (x ^ k) / Real.binEntropy (x ^ k) ^ 2 := by
      ...
      any_goals linarith;
      · unfold Real.binEntropy; norm_num [ sub_mul ] ; ring;
    ...
    · rw [ show k - 2 = k - 1 - 1 by ring, Real.rpow_sub_one hx.1.ne', Real.rpow_sub_one hx.1.ne' ] ; ring;
      field_simp;
  ...
  have h_log : deriv Real.binEntropy x = (Real.binEntropy x + Real.log (1 - x)) / x ∧ deriv Real.binEntropy (x ^ k) = (Real.binEntropy (x ^ k) + Real.log (1 - x ^ k)) / (x ^ k) := by
    have h_log : ∀ x ∈ Set.Ioo 0 1, deriv Real.binEntropy x = (Real.binEntropy x + Real.log (1 - x)) / x := by
      unfold Real.binEntropy;
      intro x hx; norm_num [ Real.log_inv, sub_mul, hx.1.ne', hx.2.ne', ne_of_gt hx.1, ne_of_gt ( sub_pos.mpr hx.2 ) ] ; ring;
  ...
  · field_simp;
    rw [ eq_div_iff ( ne_of_gt ( Real.rpow_pos_of_pos hx.1 _ ) ) ] ; ring;
...
lemma lemma_q_alpha (k : ℝ) (hk : k > 1) :
  ...
  unfold q;
  -- Using $\alpha(1+\alpha)^{k-1}=1$, we have $1/(1+\alpha)^{k-1} = \alpha$.
  have h_alpha_inv : (1 / (1 + alpha k)) ^ (k - 1) = alpha k := by
    -- Using the fact that $\alpha_k (1 + \alpha_k)^{k-1} = 1$, we can rewrite $\alpha_k$ as $1 / (1 + \alpha_k)^{k-1}$.
    have h_alpha : alpha k = 1 / (1 + alpha k) ^ (k - 1) := by
      unfold alpha;
      have := Classical.choose_spec ( show ∃ x : ℝ, x > 0 ∧ x * ( 1 + x ) ^ ( k - 1 ) = 1 from by
                                        -- We'll use the intermediate value theorem to show there exists $x \in (0, 1)$ such that $x(1+x)^{k-1} = 1$.
                                        have h_ivt : ∃ x ∈ Set.Ioo (0 : ℝ) 1, x * (1 + x) ^ (k - 1) = 1 := by
                                          apply_rules [ intermediate_value_Ioo ] <;> norm_num;
                                          · exact continuousOn_of_forall_continuousAt fun x hx => ContinuousAt.mul continuousAt_id <| ContinuousAt.rpow ( continuousAt_const.add continuousAt_id ) continuousAt_const <| Or.inl <| by linarith [ hx.1 ] ;
                                          · exact Real.one_lt_rpow ( by norm_num ) ( by linarith );
                                        aesop );
      grind;
    rw [ Real.div_rpow ] <;> norm_num at * ; linarith [ show 0 < alpha k from by { exact lt_of_le_of_ne ( by { unfold alpha; split_ifs <;> first | positivity | exact Classical.choose_spec ( _ : ∃ x : ℝ, x > 0 ∧ x * ( 1 + x ) ^ ( k - 1 ) = 1 ) |>.1.le } ) ( Ne.symm <| by { rw [ eq_comm ] at h_alpha ; aesop } ) } ] ;
    exact add_nonneg zero_le_one ( le_of_lt ( show 0 < alpha k from by { exact lt_of_le_of_ne ( by { unfold alpha; split_ifs <;> first | positivity | exact Classical.choose_spec ( _ : ∃ x : ℝ, x > 0 ∧ x * ( 1 + x ) ^ ( k - 1 ) = 1 ) |>.1.le } ) ( Ne.symm <| by { rw [ eq_comm ] at h_alpha ; aesop } ) } ) );
  rw [ show ( 1 / ( 1 + alpha k ) ) ^ k = ( 1 / ( 1 + alpha k ) ) ^ ( k - 1 ) * ( 1 / ( 1 + alpha k ) ) by rw [ ← Real.rpow_add_one ( by exact one_div_ne_zero <| by linarith [ show 0 < alpha k from by exact lt_of_le_of_lt ( by positivity ) ( lemma_l2 k hk ) ] ) ] ; ring, h_alpha_inv ];
  field_simp;
  rw [ div_eq_iff ];
  · unfold Real.binEntropy;
    field_simp;
    grind;
  · unfold Real.binEntropy;
    rw [ show ( 1 - alpha k / ( 1 + alpha k ) ) = ( 1 + alpha k ) ⁻¹ by rw [ one_sub_div ( by linarith [ show 0 < alpha k from by exact lt_of_le_of_lt ( by positivity ) ( lemma_l2 k hk ) ] ) ] ; ring ] ; norm_num;
    refine' ne_of_gt ( add_pos_of_nonneg_of_pos ( mul_nonneg ( div_nonneg ( show 0 ≤ alpha k from _ ) ( by linarith [ show 0 < alpha k from by exact lt_of_le_of_lt ( by positivity ) ( lemma_l2 k hk ) ] ) ) ( Real.log_nonneg ( by rw [ le_div_iff₀ ( show 0 < alpha k from by exact lt_of_le_of_lt ( by positivity ) ( lemma_l2 k hk ) ) ] ; linarith [ show 0 < alpha k from by exact lt_of_le_of_lt ( by positivity ) ( lemma_l2 k hk ) ] ) ) ) ( mul_pos ( inv_pos.mpr ( by linarith [ show 0 < alpha k from by exact lt_of_le_of_lt ( by positivity ) ( lemma_l2 k hk ) ] ) ) ( Real.log_pos ( by linarith [ show 0 < alpha k from by exact lt_of_le_of_lt ( by positivity ) ( lemma_l2 k hk ) ] ) ) ) );
    exact le_of_lt ( by exact lt_of_le_of_lt ( by positivity ) ( lemma_l2 k hk ) )
...
theorem generalized_boppana (k : ℝ) (hk : k > 1) (x : ℝ) (hx : 0 ≤ x ∧ x ≤ 1) :
  ...
  · -- If $M$ is attained at the boundary, then $M = 1/k < \alpha_k$, so $q_ext(x) \le \alpha_k$ for all $x \in [0, 1]$. Hence, $q(x) \le \alpha_k$ for all $x \in (0, 1)$.
    ...
    · convert mul_le_mul_of_nonneg_right ( hq_le_alpha x ( lt_of_le_of_ne hx.1 ( Ne.symm hx0 ) ) ( lt_of_le_of_ne hx.2 hx1 ) ) ( show 0 ≤ - ( x ^ k * Real.log ( x ^ k ) ) + - ( ( 1 - x ^ k ) * Real.log ( 1 - x ^ k ) ) from ?_ ) using 1;
      · unfold q; rw [ Real.log_rpow ( by linarith [ show x > 0 from lt_of_le_of_ne hx.1 ( Ne.symm hx0 ) ] ) ] ; ring;
        field_simp;
        rw [ eq_div_iff ];
        · unfold Real.binEntropy; ring;
          rw [ Real.log_inv, Real.log_inv, Real.log_rpow ( by linarith [ show x > 0 from lt_of_le_of_ne hx.1 ( Ne.symm hx0 ) ] ) ] ; ring;
          rw [ Real.log_inv, Real.log_inv ] ; ring;
        · unfold Real.binEntropy;
          norm_num;
  ...
  · -- If $M$ is attained in $(0, 1)$, then $q'(c) = 0$ for some $c \in (0, 1)$.
    ...
    · convert mul_le_mul_of_nonneg_right ( hq_ext_le_M x hx.1 hx.2 ) ( show 0 ≤ Real.binEntropy ( x ^ k ) from _ ) using 1;
      · simp +zetaDelta at *;
        rw [ if_neg ( by aesop ) ] ; unfold q ; ring;
    ...
    ∀ x ∈ Set.Icc 0 1, q_extended k x ≤ alpha k ∧ (q_extended k x = alpha k ↔ x = 1 / (1 + alpha k)) := by
      ...
      have h_l3 : ∀ x ∈ Set.Icc 0 1, q_extended k x ≤ alpha k ∧ (q_extended k x = alpha k ↔ x = 1 / (1 + alpha k)) := by
        ...
        have h_critical : q_extended k (1 / (1 + alpha k)) = alpha k := by
          ...
          · exact if_neg ( by rintro ( h | h ) <;> nlinarith [ show 0 < alpha k from by exact lt_of_le_of_lt ( by positivity ) ( lemma_l2 k hk ), one_div_mul_cancel ( by linarith [ show 0 < alpha k from by exact lt_of_le_of_lt ( by positivity ) ( lemma_l2 k hk ) ] : ( 1 + alpha k ) ≠ 0 ) ] )
        ...
        exact ⟨ by linarith [ show 0 < alpha k from by exact lt_of_le_of_lt ( by positivity ) ( lemma_l2 k hk ) ], inv_lt_one_of_one_lt₀ <| by linarith [ show 0 < alpha k from by exact lt_of_le_of_lt ( by positivity ) ( lemma_l2 k hk ) ] ⟩;
      assumption
